...
getFolder = function(folder)
	...
	local translation = folder == "Translations"
	local format = translation and "-- Translations\n\ttranslations = {\n%s\n\t},\n\tlangue = \"en\"," or "%s"
...
setFile = function()
	...
	local file = io.open(path .. string.format("m_%s_%s.lua",modeName,os.date("%y_%m_%d")),"w+")
	file:write(info)
